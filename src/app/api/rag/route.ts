...
import { runRagPipeline } from '@/contexts/legal-search/application/use-cases/RunRagPipelineUseCase'
import { RagBodySchema } from './schema'
import { getUserTier, checkUsageLimit, trackUsage, adjustQueryForTier } from '@/contexts/subscription/domain/services/TierService'
import { authenticateUser, logQuery, logQualityMetrics, assignABTestVariant, logABTest } from '@/contexts/identity/infrastructure/AuthService'
import { logger } from '@/shared/utils/Logger'
import { checkRateLimit } from '@/contexts/subscription/domain/services/RateLimitService'
import { detectLegalArea, detectComplexity, calculateCitationPrecision } from '@/contexts/legal-search/domain/services/QueryAnalyzerService'
...
function initCache() {
  try {
    ...
    const persistentCache = require('@/shared/infrastructure/PersistentCache')
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { query, filters, locale = 'es', useQueryExpansion, usePromptByArea } = parsed.data

 ...
 * Umbrales adaptados para scores RRF+reranking (escala ~0.03-0.35)
 * que son mucho menores que cosine similarity puro (0-1).
 */
...
import { generateAnswerSpanish } from '@/contexts/legal-search/infrastructure/llm/GenerationService'
import { filterSensitivePII } from '@/shared/utils/PiiFilter'
import { type RagQuery, type RagResponse, type DocumentChunk } from '@/shared/types'
import { detectLegalArea, detectComplexity } from '@/contexts/legal-search/infrastructure/llm/PromptTemplates'
import { logger } from '@/shared/utils/Logger'
import { isProcedureRelatedQuery, getProcedureChunksForQuery } from '@/contexts/legal-knowledge/domain/services/ProceduresService'
import { consultarVigencia, inferNormaIdFromTitle } from '@/contexts/legal-knowledge/domain/services/NormVigenciaService'
import { shouldUseRecursiveRag, runRecursiveRag, type RecursiveRagConfig } from '@/contexts/legal-search/application/use-cases/RunRecursiveRagUseCase'
import { extractApplicableNorms } from '@/contexts/legal-knowledge/domain/services/NormExtractorService'
import { validateLogicCoherence, generateCoherenceFeedback } from '@/contexts/answer-quality/domain/services/LogicValidatorService'
import { compareSources } from '@/contexts/answer-quality/domain/services/SourceComparatorService'
import { explainLegalHierarchy } from '@/contexts/legal-knowledge/domain/services/LegalHierarchyService'
import { getOrCreateTrace, startStep, endStep, setTraceResult } from '@/contexts/analytics/infrastructure/TracingService'

// Lazy load heavy modules to optimize cold starts
// These are only imported when actually needed
type FactualValidator = typeof import('@/contexts/answer-quality/domain/services/FactualValidatorService')
type ResponseStructure = typeof import('@/contexts/legal-search/domain/value-objects/HnacStructure')
type LegalCalculator = typeof import('@/contexts/legal-knowledge/domain/services/LegalCalculatorService')
type CitationValidator = typeof import('@/contexts/answer-quality/domain/services/CitationValidatorService')

// Import type for calculations
import type { CalculationResult } from '@/contexts/legal-knowledge/domain/services/LegalCalculatorService'
...
async function performCalculations(
  ...
  try {
    ...
    } = await import('@/contexts/legal-knowledge/domain/services/LegalCalculatorService') as LegalCalculator
...
export async function runRagPipeline(params: RagQuery): Promise<RagResponse> {
  ...
  const {
    ...
    userId,
    useQueryExpansion: useQueryExpansionParam,
    usePromptByArea: usePromptByAreaParam
  } = params
  ...
  // Top-K adaptativo según complejidad (S8.2: más contexto para artículos correctos)
  const adaptiveTopK = detectedComplexity === 'alta' ? 20 : detectedComplexity === 'media' ? 16 : 12
  ...
  const retrieved = await retrieveRelevantChunks(query, filters, adaptiveTopK, useQueryExpansionParam !== undefined ? { useQueryExpansion: useQueryExpansionParam } : undefined)
  ...
  if (enableFactualValidation) {
    ...
    const { validateFactual } = await import('@/contexts/answer-quality/domain/services/FactualValidatorService') as FactualValidator
  ...
  if (enableCitationValidation) {
    ...
    const { validateCitations } = await import('@/contexts/answer-quality/domain/services/CitationValidatorService') as CitationValidator
  ...
  if (enableStructuredResponse) {
    ...
    const { structureResponse } = await import('@/contexts/legal-search/domain/value-objects/HnacStructure') as ResponseStructure

...
/**
 * Cross-encoder real vía API HF (S8.1). Usa BAAI/bge-reranker-v2-m3 por defecto.
 * Llama POST .../models/BAAI/bge-reranker-v2-m3 con { inputs: { source_sentence: query, sentences: [...] } }.
...
async function getCrossEncoderScores(
  ...
  try {
    // S8.1: Cross-encoder real vía HuggingFace Inference API (no sentenceSimilarity bi-encoder)
    ...
    if (!res.ok) {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.warn('[rerank] Cross-encoder API error:', res.status, errText.slice(0, 200))
      }
      return null
    }

    const scores = (await res.json()) as number[] | { error?: string }
    if (Array.isArray(scores) && scores.length === toRank.length) {
      // Scores pueden ser logits (bge-reranker); normalizar a 0-1
    ...
    // Algunos modelos devuelven un objeto; intentar extraer scores
    if (typeof scores === 'object' && scores !== null && !Array.isArray(scores) && 'error' in scores) {
      return null
    }
    return null
  } catch {

...
import { embedText } from '@/contexts/legal-search/infrastructure/embeddings/EmbeddingService'
import { type DocumentChunk, type RetrieveFilters } from '@/shared/types'
import { applyReranking, applyRerankingWithCrossEncoder, rerankWithHFSimilarity, addDerogadaNoteToChunk } from '@/contexts/legal-search/domain/services/RerankingService'
import { calculateBM25, hybridScore, deserializeBM25Index, searchBM25, rrfMerge, type BM25Index } from '@/contexts/legal-search/infrastructure/sparse-search/Bm25SearchService'
import { isHNSWAvailable, loadHNSWIndex, searchHNSW, getHNSWIdListPath, RRF_K } from '@/contexts/legal-search/infrastructure/vector-store/HnswVectorStore'
import { consultarVigencia, inferNormaIdFromTitle } from '@/contexts/legal-knowledge/domain/services/NormVigenciaService'
import { expandQuery, detectLegalArea } from '@/contexts/legal-search/domain/services/QueryExpansionService'
...
function applyMetadataBoost<T extends { chunk: DocumentChunk; score: number }>(
  ...
  const detectedAreaLower = detectedArea.toLowerCase()
  const keywords = areaKeywords[detectedAreaLower] || []

  // Keywords negativos por área: normas que NO deberían dominar para ciertas queries
  const negativeKeywordsByArea: Record<string, string[]> = {
    // Para consultas laborales queremos priorizar CST sobre CPACA / ET / Comercio
    laboral: [
      'cpaca',
      ...
      'código cpaca',
      'estatuto tributario',
      'codigo estatuto tributario',
      'código estatuto tributario',
      'codigo de comercio',
      'código de comercio'
    ],
  }
  const negativeKeywords = negativeKeywordsByArea[detectedAreaLower] || []
  ...
  const isQueryNonPenal = NON_PENAL_AREAS.includes(detectedAreaLower)
  ...
  const boosted = retrieved.map(item => {
    ...
    // Penalización adicional específica por área (ej. consultas laborales vs CPACA/ET)
    if (boostFactor >= 1.0 && negativeKeywords.length > 0) {
      for (const neg of negativeKeywords) {
        if (title.includes(neg)) {
          // No eliminar totalmente, pero bajar su prioridad de forma significativa
          boostFactor = 0.4
          break
        }
      }
...
export async function retrieveRelevantChunks(
  ...
  topK = 12,
  options?: { useQueryExpansion?: boolean }
): Promise<Array<{ chunk: DocumentChunk; score: number }>> {
  // FASE 1.3: Query expansion (coloquial → legal). S7.1 A/B: request-level override
  const useQE = options?.useQueryExpansion !== undefined ? options.useQueryExpansion : USE_QUERY_EXPANSION
  const expandedQuery = useQE ? expandQuery(query) : query
  ...
  // Retrieve more chunks initially if re-ranking is enabled (S8.2: topK*2, cap 40 for alta=20)
  const initialTopK = USE_RERANKING ? Math.min(topK * 2, 40) : topK
  ...
  if (USE_RERANKING && retrieved.length > 0) {
    ...
    console.log(`[retrieval] Reranking: provider=${RERANK_PROVIDER}, hf_key=${process.env.HUGGINGFACE_API_KEY ? 'set' : 'unset'}`)
    if (RERANK_PROVIDER === 'hf' && process.env.HUGGINGFACE_API_KEY) {
      console.log('[retrieval] Using cross-encoder: BAAI/bge-reranker-v2-m3')

 ...
 *   --no-query-expansion  S7.1 A/B: desactivar query expansion en requests al RAG
 *   --no-area-prompts    S7.2 A/B: desactivar prompts por área (usar genérico)
 *   --pause <ms>         Pausa entre casos (default: 2000; con Groq 3500 para evitar 429). Ver docs/benchmarks/RATE_LIMIT_GROQ_JUEZ.md
 *   --concurrency <n>    Workers en paralelo (default 1; con Groq mantener 1)
 *
 * Juez (LLM-as-judge):
...
// Cargar .env.local al inicio (desde repo root) para que GROQ_API_KEY esté disponible
...
// Script está en scripts/evaluation/ → subir dos niveles para repo root
...
/** S7.1 A/B: desactivar query expansion en cada request al RAG */
const NO_QUERY_EXPANSION = hasFlag('--no-query-expansion');
/** S7.2 A/B: desactivar prompts por área (prompt genérico) */
const NO_AREA_PROMPTS = hasFlag('--no-area-prompts');
...
async function queryRAG(question, retries = 3) {
  ...
  const body = {
    query: question,
    ...(NO_QUERY_EXPANSION && { useQueryExpansion: false }),
    ...(NO_AREA_PROMPTS && { usePromptByArea: false }),
  };
...
async function callJudge(toonPrompt, retries = 5) {
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    ...
    // Rate limit 429: reintentar con backoff (Groq típico: 30 RPM)
    if (res.status === 429) {
      const retryAfter = res.headers.get('retry-after');
      const waitMs = retryAfter ? parseInt(retryAfter, 10) * 1000 : Math.min(1000 * Math.pow(2, attempt), 60_000);
      if (attempt < retries) {
        process.stdout.write(yellow(` (429 rate limit juez, esperando ${Math.ceil(waitMs / 1000)}s reintento ${attempt}/${retries})... `));
        await new Promise(r => setTimeout(r, waitMs));
        continue;
      }
      throw new Error(`Judge API rate limit (429) tras ${retries} reintentos. Usa --pause 3500 o más. Ver docs/benchmarks/RATE_LIMIT_GROQ_JUEZ.md`);
    }
...
async function main() {
  ...
  const CONCURRENCY = getArg('--concurrency', '1'); // Siempre 1 con Groq para evitar 429
  const defaultPauseMs = (process.env.JUDGE_PROVIDER || '').toLowerCase() === 'groq' ? '3500' : '2000';
  const PAUSE_MS = parseInt(getArg('--pause', defaultPauseMs)); // Groq ~30 RPM → 3.5s entre requests
  const SAVE_EVERY = 5; // guardar checkpoint más frecuente
  console.log(gray(`\n  ⚡ Concurrencia: ${CONCURRENCY} | Pausa: ${PAUSE_MS}ms${defaultPauseMs === '3500' ? yellow(' (Groq: evita 429)') : ''} | Timeout RAG: 180s (2 intentos)\n`));

...
/**
 * S8.3: Prefijo de jerarquía normativa para contextual chunking.
 * Formato: "[Norma > Título X > Capítulo Y > Art. N] "
 * Solo se agrega si el contenido no empieza ya con "[" (evitar duplicado).
 */
function buildHierarchyPrefix(title, part) {
  if (!title) return ''
  let display = title
  if (part.title) display += ' > ' + part.title
  if (part.chapter) display += ' > ' + part.chapter
  if (part.section) display += ' > ' + part.section
  if (part.article) {
    const artShort = part.article.replace(/Art[íi]culo\s+/i, 'Art. ')
    display += ' > ' + artShort
  }
  return '[' + display + '] '
}
...
async function main() {
  ...
  for (const file of files) {
    ...
    for (const part of articleChunks) {
      // FASE_2 2.1 + S8.3: Jerarquía Ley > Título > Capítulo > Artículo (metadata y prefijo contextual)
      ...
      // S8.3: Prefijo en formato [Norma > Título > Capítulo > Art. N] para embedding y LLM
      const hierarchyPrefix = buildHierarchyPrefix(title, part)
      ...
      for (const content of contentParts) {
        ...
        // S8.3: Prepend hierarchy solo si el chunk no tiene ya formato [ ... ]
        const contentWithPrefix = raw.startsWith('[') ? raw : hierarchyPrefix + raw
  ...
  if (process.env.PINECONE_API_KEY && process.env.PINECONE_INDEX) {
    ...
    stream.setMaxListeners(1000) // Evita MaxListenersExceededWarning en ingest largo (900+ batches)

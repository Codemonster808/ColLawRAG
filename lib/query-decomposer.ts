...
/**
 * Tipos de complejidad de consulta
 */
export type QueryComplexity = 'simple' | 'multi' | 'comparative';

/**
 * Indicador de múltiples partes detectado
 */
export interface MultiPartIndicator {
...
/**
 * Resultado del análisis de consulta
 */
export interface QueryAnalysis {
  ...
  confidence: number; // 0-1 (confianza en que es multi-parte)
  metadata: {
 ...
 */
function detectIndicators(query: string): MultiPartIndicator[] {
  const indicators: MultiPartIndicator[] = [];
 ...
 */
function splitIntoParts(query: string, indicators: MultiPartIndicator[]): string[] {
 ...
 */
function determineComplexity(indicators: MultiPartIndicator[], parts: string[]): QueryComplexity {
  const hasComparison = indicators.some(ind => ind.type === 'comparative');
 ...
 */
function calculateConfidence(indicators: MultiPartIndicator[], parts: string[]): number {
  if (indicators.length === 0) {
    return 0.0;
  }
 ...
 */
export function detectMultiPart(query: string): {
  isMultiPart: boolean;
  parts: string[];
  complexity: QueryComplexity;
} {
